...
@objc public extension UIDevice {
    /// UIDevice Width
    @objc static var YH_Width: CGFloat {
        return UIScreen.main.bounds.size.width
    }
    
    /// UIDevice Height
    @objc static var YH_Height: CGFloat {
        return UIScreen.main.bounds.size.height
    }
    
    // Get machine name, such as `iPhone 7 Plus`.
    @objc static var YHMachineName: String {
    ...
    @objc static func YHBasicDeviceInfo() -> String {
    ...
    @objc static var YH_Is_Fringe: Bool {
        let machine = UIDevice.YHMachine
    ...
    @objc static var YH_Nav_Height: CGFloat {
        return UIDevice.YH_Fringe_Height + 44.0
    }
    ...
    @objc static var YH_Is_HomeIndicator: Bool {
        let machine = UIDevice.YHMachine
        guard let map = UIDevice.YHMachineMap[machine] else { return false }
        var result: Bool = false
        if map == .iPhoneX ||
    ...
    @objc static var YH_SimulatorIsiPhoneX: Bool {
        let machine = UIDevice.YHMachine
        guard let map = UIDevice.YHMachineMap[machine] else { return false }
        if map != .simulator {
            return false
